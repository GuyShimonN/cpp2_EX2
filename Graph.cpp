...
size_t Graph::getNumberOfNodes() const {
    ...
    std::vector <std::pair<size_t, std::pair < size_t, int>>>
    ...
    int Graph::getEdgeWeight(size_t ver1, size_t ver2) const {
        if (static_cast<size_t>(ver1) >= adjacencyMatrix.size() ||
            static_cast<size_t>(ver2) >= adjacencyMatrix.size()) {
            throw std::out_of_range("Node index out of range");
        }
...
namespace ariel {
    Graph Graph::operator+(const Graph &other) const {
        ...
        Graph result;
        std::vector <std::vector<int>> resultMatrix(this->getNumberOfNodes(),
                                                    std::vector<int>(this->getNumberOfNodes(), 0));
    ...
    Graph &Graph::operator*=(const Graph &other) {
        ...
        std::vector <std::vector<int>> resultMatrix(n, std::vector<int>(n,
                                                                        0)); // Initialize the result adjacency matrix with zeros
